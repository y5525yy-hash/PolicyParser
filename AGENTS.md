...
The visual source of truth is the manually adjusted Figma frame `首页-最终版`, node `52:2`:

`https://www.figma.com/design/LFju2LeTMMmQCtilcZ4pQa?node-id=52-2`

Use the following standards across all later pages:
...
- Desktop content width is 1152px inside a 1280px reference canvas.
- Use 8px control radii, 12px card radii, and 16px primary-panel radii.
- Standard card shadow: `0 4px 12px rgba(15, 31, 51, 0.08)`.
- Do not reintroduce warm beige, the old dark-green theme, decorative gradients, complex animation, or unnecessary imagery.
- Keep primary actions visually dominant and preserve empty, loading, and error state locations.
- Reuse the homepage variables and established components on every later page.
