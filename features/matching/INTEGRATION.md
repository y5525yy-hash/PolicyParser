...
- `llm-resident-evaluation.server.ts`：使用 5 名人工构造居民和冻结答案，先做离线基线，再通过测试字段元数据验证真实模型对齐；模型请求不含居民姓名、ID 或字段值。
- `scripts/verify-synthetic-residents.cjs`：合成居民真实评测入口，单次运行最多 30 次请求，当前 21 条条件在一批请求中完成。
...
合成居民的人工答案位于 `synthetic-resident-ground-truth.ts`，必须先于模型调用冻结。新增居民数据位于 B 模块的 `synthetic-residents.ts`，目前补充了家庭特殊身份和残疾人证两个政策事实字段。
