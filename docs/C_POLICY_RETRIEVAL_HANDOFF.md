...
- 已保存 35 份北京市官方政策完整文本，对应 31 个稳定 `policyId`，覆盖养老、社会救助、残疾人、儿童、住房、就业、医疗、教育、法律援助和家庭扶助等官方分类。
- 已生成 1396 个包含来源链路的政策切片，其中 1381 个进入默认有效政策检索。
...
- `policy-004`、`policy-005` 已补入基础参保政策，年度缴费文件继续作为当年金额依据，参保条件和缴费金额必须分别引用对应原文。
...
- `policy-004`：已补入京人社居发〔2014〕177号，参保资格与按月领取待遇是两组不同条件，不能混为一项判断。
- `policy-005`：已补入京人社农合发〔2017〕250号完整参保范围；各人群分支的户籍、年龄、学籍和“无其他基本医疗保障”要求不同。
...
- `policy-026`：已补入独生子女死亡家庭和伤残家庭两份基础实施方案；两类资格是独立分支，分支内部条件必须同时满足。
...
- 不要把 `policy-004` 的参保条件当成领取养老金条件，也不要把 `policy-005` 的三个年度缴费金额当成完整参保人群定义。
